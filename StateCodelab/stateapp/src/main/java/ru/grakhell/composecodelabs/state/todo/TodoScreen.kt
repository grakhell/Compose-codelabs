...
fun TodoInputTextField(
    ...
    TodoInputText(
        text, onTextChange, modifier, onImeAction
    )
}
...
fun TodoItemEntryInput(onItemComplete: (TodoItem) -> Unit) {
    ...
    TodoItemInput(text = text, onTextChanged = setText, icon = icon, onIconChanged = setIcon, submit = submit, iconsVisible = iconsVisible) {
        Row {
            TodoEditButton(
                onClick = submit,
                text = "Add",
...
fun TodoItemInput(
    ...
    Column {
        ...
        if (iconsVisible) {
            AnimatedIconRow(icon, onIconChanged, Modifier.padding(top = 8.dp))
        } else {
...
fun TodoRow(
    ...
    alpha: Float = remember(todo.id) { randomTint() }
) {
    Row(
        modifier = modifier
